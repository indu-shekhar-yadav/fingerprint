import serial
import datetime
import json
import openpyxl
import os

# Establish serial connection with Arduino
ser = serial.Serial('COM4', 9600)  # Replace 'COM4' with the appropriate serial port

attendance_data = []
recorded_ids = set()

while True:
    # Read data from Arduino
    data = ser.readline().decode().strip()
    if data:
        fingerprint_id = data.split()[0]
        print(fingerprint_id)

        if fingerprint_id == "5":
            fingerprint_id = "Indu"
        elif fingerprint_id == "2":
            fingerprint_id = "soumyajit"

        elif(fingerprint_id=="4"):
            fingerprint_id="Teacher locked the attandance"
            
        else:
            fingerprint_id = "Shival"

        # Check if the fingerprint ID has already been recorded
        if fingerprint_id not in recorded_ids:
            recorded_ids.add(fingerprint_id)

            # Get the current timestamp
            timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')

            # Create a dictionary to store the fingerprint data
            fingerprint = {"id": fingerprint_id, "timestamp": timestamp ,}

            # Append the fingerprint data to the attendance list
            attendance_data.append(fingerprint)

            # Save the attendance data to a JSON file
            with open("fingerprint_data.json", "w") as json_file:
                json.dump(attendance_data, json_file)

            # Save the attendance data to an Excel file
            if os.path.isfile("fingerprint_data.xlsx"):
                workbook = openpyxl.load_workbook("fingerprint_data.xlsx")
            else:
                workbook = openpyxl.Workbook()
                workbook.active['A1'] = "ID"
                workbook.active['B1'] = "Timestamp"
            
            sheet = workbook.active
            existing_ids = [sheet.cell(row=row, column=1).value for row in range(2, sheet.max_row + 1)]
            
            if fingerprint_id not in existing_ids:
                row = sheet.max_row + 1
                sheet.cell(row=row, column=1, value=fingerprint_id)
                sheet.cell(row=row, column=2, value=timestamp)
                sheet.cell(row=row, column=3, value="Present")
            
            workbook.save("fingerprint_data.xlsx")


    if fingerprint_id=="Teacher locked the attandance":
        break
